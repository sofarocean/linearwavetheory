...
def test_amplitude_coef():
    ...
    for ii, freq in enumerate(freqs):
        ...
        #
        epsilon = k[ii] * 1
        stokes[ii] = 4 * epsilon**3 * a31(k[ii] * depth) / k[ii]
...
def test_lagrangian_amplitude_coef():
    depth = 10
    kd = np.linspace(1, 6, 301, endpoint=True)
    k = kd / depth
    omega = intrinsic_dispersion_relation(k, depth)
    ...
    bound = np.zeros_like(freqs)
    stokes = np.zeros_like(freqs)
    include_mean_setdown, include_mean_flow, lagrangian = True, True, True
    ...
    for ii, freq in enumerate(freqs):
        ...
        #
        epsilon = k[ii] * 1
        stokes[ii] = 4 * epsilon**3 * eta31(k[ii] * depth, 0) / k[ii]
