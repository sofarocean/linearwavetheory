...
def test_inverse_intrinsic_dispersion_relation():
    ...
    assert_allclose(
        intrinsic_frequency, solution_freq, atol=0, rtol=settings.RELATIVE_TOLERANCE
    )
    ...
    solution_freq = ld.intrinsic_dispersion_relation(solution, 10)
    assert_allclose(
        solution_freq, intrinsic_frequency, atol=0, rtol=settings.RELATIVE_TOLERANCE
    )
...
def test_intrinsic_dispersion_relation():
    ...
    shallow_water_omega = k * np.sqrt(settings.GRAV * depth)
    omega = ld.intrinsic_dispersion_relation(k, depth)

    assert_allclose(
        omega, shallow_water_omega, atol=0, rtol=settings.RELATIVE_TOLERANCE
    )

    # Deep water
    k = np.linspace(0.1, 1, 100)
    depth = np.full(100, 1000.0)
    deep_water_omega = np.sqrt(settings.GRAV * k)
    omega = ld.intrinsic_dispersion_relation(k, depth)
    assert_allclose(omega, deep_water_omega, atol=0, rtol=settings.RELATIVE_TOLERANCE)

    # Capillary waves
    k = np.linspace(10000, 100000, 10)
    depth = np.full(10, 1000.0)
    capillary_omega = np.sqrt(settings.KINEMATIC_SURFACE_TENSION * k**3)
    omega = ld.intrinsic_dispersion_relation(k, depth)
    assert_allclose(
        omega, capillary_omega, atol=np.inf, rtol=settings.RELATIVE_TOLERANCE
    )
...
def test_intrinsic_phase_speed():
    ...
    shallow_water_c = np.sqrt(settings.GRAV * depth)
    c = ld.intrinsic_phase_speed(k, depth)

    assert_allclose(c, shallow_water_c, atol=0, rtol=settings.RELATIVE_TOLERANCE)
    ...
    k = np.linspace(10000, 100000, 10)
    depth = np.full(10, 1000.0)
    capillary_omega = np.sqrt(settings.KINEMATIC_SURFACE_TENSION * k)
    omega = ld.intrinsic_phase_speed(k, depth)
    assert_allclose(omega, capillary_omega, atol=0, rtol=settings.RELATIVE_TOLERANCE)
...
def test_intrinsic_group_speed():
    ...
    shallow_water_c = np.sqrt(settings.GRAV * depth) * np.ones(100)
    c = ld.intrinsic_group_speed(k, depth)
    assert_allclose(c, shallow_water_c, atol=0, rtol=settings.RELATIVE_TOLERANCE)
    ...
    k = np.linspace(0.1, 1, 100)
    depth = np.full(100, 1000.0)
    deep_water_omega = np.sqrt(settings.GRAV / k) / 2
    ...
    k = np.linspace(10000, 100000, 10)
    depth = np.full(10, 1000.0)
    capillary_omega = 3 * np.sqrt(settings.KINEMATIC_SURFACE_TENSION * k) / 2
    omega = ld.intrinsic_group_speed(k, depth)
    assert_allclose(omega, capillary_omega, atol=0, rtol=settings.RELATIVE_TOLERANCE)
